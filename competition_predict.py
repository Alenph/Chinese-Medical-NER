...
with open('./best_ckpt_path1.txt', 'r', encoding='utf-8') as f:
    CKPT_PATH = f.read().strip()

"""
由于路径读取的问题，./bast_ckpt_path1.txt为手动添加的数据，数据内容为训练好的模型路径,内容拷贝与bast_skpt_path
EX:
/home/pengh/py_home/NLP_home/experiment/DeepNER-master/out/roberta_wwm_wd_crf/checkpoint-816/model.pt
"""
...
def single_predict():
    ...
    # i=0
    # CKPT_PATH1=[]
    # print(len(CKPT_PATH.splitlines()))
    # for i in range(len(CKPT_PATH.splitlines())):
    #    CKPT_PATH1.append('../.'+CKPT_PATH.splitlines()[i]+'\n')
    #
    # CKPT_PATH2=''
    # CKPT_PATH2=''.join(CKPT_PATH1)
    # print(CKPT_PATH2.splitlines()[1])
...
def mixed_predict():
    ...
    为测试集中文件的大小，在测试集大小修改后，要记得修改range()大小
    """
    #for key in range(1500, 1997):
    for key in range(1093, 1997):
        with open(os.path.join(save_dir, f'{key}.ann'), 'w', encoding='utf-8') as f:
            if not len(labels[key]):
                print(key)
                f.write("")
